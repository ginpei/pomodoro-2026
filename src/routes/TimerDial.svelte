<script lang="ts">
  ...
  import { DEFAULT_BREAK_DURATION, type TimerState } from '$lib/timer';

  export let value: TimerState;

  const dispatch = createEventDispatcher<{ setProgress: number }>();
  ...
  function deriveStateForProgress(state: TimerState, progress: number): TimerState {
    ...
    const remaining = clamp(
      Math.round(DEFAULT_BREAK_DURATION - breakElapsed),
      0,
  ...
  function updateDrag(event: PointerEvent) {
    ...
    if (distance > RADIUS + OUTSIDE_THRESHOLD) {
      ...
      dragCanceled = false;
      dragProgress = progress;
      dragPreview = deriveStateForProgress(dragBaseState ?? value, progress);
    }
  }
  ...
  function endDrag(event: PointerEvent) {
    ...
    if (!dragCanceled && dragProgress != null) {
      dispatch('setProgress', dragProgress);
    }
    dragActive = false;
    dragCanceled = false;
    dragPointerId = null;
  ...
  $: displayValue = dragPreview ?? value;
